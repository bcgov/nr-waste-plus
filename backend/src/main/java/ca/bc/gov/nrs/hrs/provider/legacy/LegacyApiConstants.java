...
/**
 * Contains default values for various entities including districts, codes, and search results that
 * are used as fallback values when the legacy API is unavailable or returns invalid data.
 *
 * <p>Centralizing these constants avoids duplication in the provider and makes them easier to reuse
 * and test.
 */
...
public final class LegacyApiConstants {
   ...
   *
   * <p>This constant serves as a fallback when the legacy API's district codes endpoint is
   * unavailable. It contains all standard BC forest districts.
   */
  public static final List<CodeDescriptionDto> DEFAULT_DISTRICTS =
      List.of(
   ...
   *
   * <p>Used as a fallback for sampling codes and assessment area status codes when the legacy API
   * is unavailable.
   */
   ...
   *
   * <p>Legacy API paged responses contain a "page" field with pagination metadata such as total
