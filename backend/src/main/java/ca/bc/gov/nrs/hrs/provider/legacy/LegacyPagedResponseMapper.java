 ...
 * responses, which are typically structured as JSON with {@code content} and {@code page} fields.
 *
 * <p>The mapper handles:
 *
 * <ul>
 *   <li>Validation of paged response structure
 *   <li>Extraction and conversion of content to typed lists
 *   <li>Reading pagination metadata (total elements, etc.)
 * </ul>
 */
@Component
...
public class LegacyPagedResponseMapper {
   ...
   *
   * <p>A valid paged response must contain both {@code content} and {@code page} fields. This
   * method returns {@code true} if the response is null or missing either required field.
   *
   * @param pagedResponse the JSON response to validate, may be null
   * @return {@code true} if the response is invalid or missing required structure, {@code false}
   *     otherwise
   */
   ...
   *
   * <p>This method reads the {@code content} field from the JSON response and converts it to a list
   * of objects of the specified element type using Jackson's type mapping capabilities.
   *
   ...
   *
   * <p>This method reads the {@code totalElements} field from the {@code page} object in the paged
   * response. If the field is missing or cannot be read, defaults to 0.
   *
  ...
  public long readTotalElements(JsonNode pagedResponse) {
    return pagedResponse.get(LegacyApiConstants.PAGE_CONST).get("totalElements").asLong(0L);
  }
}
