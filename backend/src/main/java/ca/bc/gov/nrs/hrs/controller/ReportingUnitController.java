 ...
 * contract this endpoint does not return a response body.
 */
@RestController
...
public class ReportingUnitController {
   ...
   *
   * <p>Delegates to {@link ReportingUnitService} to fetch and enrich the reporting unit data.
   * Returns a combined view including client information, status, grade, sampling, and district.
   *
   * <p>Returns HTTP 404 when {@link FeatureFlag#REPORTING_UNIT_DETAILS_ENABLED} is disabled.
   *
   * <p>Validates that the authenticated user has permission to access the specified reporting unit.
   * For BCeID users, validates that the reporting unit's client number matches one of the user's
   ...
   *
   * <p>Creates a reporting unit in the legacy system and returns HTTP 201 (Created) with a Location
   * header pointing to the frontend resource (/reporting-units/{id}). Per API contract, this
   * endpoint does not return a response body.
   *
