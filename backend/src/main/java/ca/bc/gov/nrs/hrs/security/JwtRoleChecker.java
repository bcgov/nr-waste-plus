...
/**
 * Utility bean used from SpEL expressions to check the currently
 * authenticated principal's roles and identity provider.
 *
 * <p>
 * Provides convenience methods used in {@code @PreAuthorize} annotations
 * (for example: {@code @auth.hasConcreteRole(@roles.VIEWER)}).
 * </p>
 */
@Component
public class JwtRoleChecker {
  ...
  /**
   * Checks whether the authenticated principal has the supplied role or an
   * abstract role derived from it (e.g. {@code ROLE_PREFIX_12345}).
   *
  ...
  /**
   * Check whether the authenticated principal has an abstract role built
   * from the provided prefix and client id (prefix_clientId).
   *
  ...
  /**
   * Check roles using an arbitrary predicate against the upper-cased
   * authority string.
   *
  ...
  /**
   * Check whether the authenticated principal's identity provider matches
   * the supplied enum value.
   *
