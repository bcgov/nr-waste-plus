...
/**
 * Expression evaluator using the exp4j library with custom mathematical and logical functions.
 */
public final class FormulaEvaluatorExp4j {
  ...
  private static final Function LN =
      new Function("ln", 1) {
        @Override
        public double apply(double... args) {
          return Math.log(args[0]);
  ...
  private static final Function ABS =
      new Function("abs", 1) {
        @Override
        public double apply(double... args) {
  ...
  private static final Function CEIL =
      new Function("ceil", 1) {
        @Override
        public double apply(double... args) {
  ...
  private static final Function FLOOR =
  ...
  private static final Function POW =
      new Function("pow", 2) {
        @Override
        public double apply(double... args) {
  ...
  private static final Function IF =
      new Function("if", 3) {
        @Override
        public double apply(double... args) {
  ...
  private static final Operator GTE =
      new Operator(">=", 2, true, Operator.PRECEDENCE_ADDITION - 1) {
        @Override
        public double apply(double... values) {
          return values[0] >= values[1] ? 1.0 : 0.0;
  ...
  private static final Operator LTE =
      new Operator("<=", 2, true, Operator.PRECEDENCE_ADDITION - 1) {
        @Override
        public double apply(double... values) {
          return values[0] <= values[1] ? 1.0 : 0.0;
  ...
  /**
   * Evaluates a mathematical formula expression using provided variable bindings.
   *
   * @param expression the mathematical formula expression to evaluate
   * @param variables map of variable names to their BigDecimal values
   * @return the evaluated result as a scaled BigDecimal
   */
    ...
    try {
      Expression exp =
          new ExpressionBuilder(expression)
              ...
              .functions(
                  SQRT, SIN, COS, TAN, ASIN, ACOS, ATAN, LOG, LN, EXP, ABS, CEIL, FLOOR, POW, MAX,
                  MIN, IF)
              .operator(GT)
              .operator(LT)
              .operator(GTE)
