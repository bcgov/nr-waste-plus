 ...
 * sampling, and assessment area status codes. It implements resilience patterns via circuit breaker
 * to gracefully handle failures by returning fallback values.
 *
 * <p>All methods are annotated with {@link
 * io.github.resilience4j.circuitbreaker.annotation.CircuitBreaker} to provide automatic fault
 * tolerance, falling back to default or empty lists when the legacy API is unavailable.
 */
@Slf4j
@Component
@Observed
public class LegacyCodesClient {
   ...
   *
   * <p>Fetches a list of all available district codes with their descriptions. This method is
   * protected by a circuit breaker that will return {@link LegacyApiConstants#DEFAULT_DISTRICTS} if
   * the API is unavailable or fails.
   *
   * <p>The legacy API endpoint is: {@code GET /api/codes/districts}
   *
   * @return a list of {@link CodeDescriptionDto} representing all available districts; never null,
   *     defaults to {@link LegacyApiConstants#DEFAULT_DISTRICTS} if API fails
   ...
   *
   * <p>Fetches a list of all available sampling codes with their descriptions. This method is
   * protected by a circuit breaker that will return an empty list if the API is unavailable or
   * fails.
   *
   * <p>The legacy API endpoint is: {@code GET /api/codes/samplings}
   *
   * @return a list of {@link CodeDescriptionDto} representing all available sampling codes; never
   *     null, defaults to empty list if API fails
   ...
   * This method is protected by a circuit breaker that will return an empty list if the API is
   * unavailable or fails.
   *
   * <p>The legacy API endpoint is: {@code GET /api/codes/assess-area-statuses}
   *
   * @return a list of {@link CodeDescriptionDto} representing all available assessment area status
   *     codes; never null, defaults to empty list if API fails
   * @see LegacyApiConstants#CODE_LIST
   */
  @CircuitBreaker(name = "breaker", fallbackMethod = "fallbackEmptyList")
  @NewSpan
   ...
   *
   * <p>Returns a predefined list of default districts to ensure the application continues operating
   ...
   *
   * <p>Returns an empty list to allow the application to continue operating when the API is
   * unavailable for sampling codes or status codes.
   *
   * @param throwable the exception that triggered the fallback, may be null
