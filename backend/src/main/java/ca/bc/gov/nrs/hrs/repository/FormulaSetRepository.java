...
public interface FormulaSetRepository extends JpaRepository<FormulaSetEntity, Long> {
  /** Finds non-deleted formula sets with a count of their non-deleted rows. */
  @Query(value = "select new ca.bc.gov.nrs.hrs.dto.formula.FormulaSetListItemDto("
      + "f.id, f.area, f.startDate, f.endDate, f.deleted, "
      + "(select count(r) from FormulaSetRowEntity r "
      + "where r.formulaSetId = f.id and r.deleted = false), "
      + "f.createdAt, f.updatedAt) "
      + "from FormulaSetEntity f where f.deleted = false "
      + "order by f.startDate desc, f.id desc",
      countQuery = "select count(f) from FormulaSetEntity f where f.deleted = false")
  Page<ca.bc.gov.nrs.hrs.dto.formula.FormulaSetListItemDto> findAllLive(Pageable pageable);
  ...
  List<FormulaSetEntity> findPredecessorForReopen(
      @Param("area") Area area, @Param("date") LocalDate date);
}
