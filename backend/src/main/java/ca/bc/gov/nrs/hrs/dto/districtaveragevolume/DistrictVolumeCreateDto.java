 ...
 *
 * <p>This DTO contains both metadata and a polymorphic table structure that defines district-level
 * volume data for either INTERIOR or COASTAL areas.
 *
 * <p>The {@code tableData} field is polymorphic and uses a Jackson "type" discriminator to
 * determine the correct subtype during serialization/deserialization.
 *
 * <p>Validation rules:
 *
 * <ul>
 *   <li>{@code area} must be either "INTERIOR" or "COASTAL"
 *   <li>{@code startDate} must be strictly in the future (@Future and service-level check)
 *   <li>{@code heliMultiplier} is required when {@code area = "COASTAL"} (enforced in the service
 *       layer)
 * </ul>
 */
