...
/**
 * Utility helpers for building URI query parameters, particularly for multi-value query parameters
 * and pageable parameters.
 *
 * <p>These helpers return {@link MultiValueMap} instances suitable for use with Spring's {@code
 * UriComponentsBuilder#queryParams()} calls.
 */
...
public class UriUtils {
  ...
  /**
   * Convert a {@link Pageable} into a MultiValueMap of query parameters which include page, size
   * and any sort orders.
   *
   * @param page the pageable to convert
   * @return MultiValueMap of pageable query parameters
   */
  public static MultiValueMap<String, String> buildPageableQueryParam(Pageable page) {
    ...
    if (page != null) {
      ...
      if (page.getSort().isSorted()) {
        page.getSort()
            .forEach(order -> multiValueMap.add("sort", order.toString().replace(": ", ",")));
      }
    }
