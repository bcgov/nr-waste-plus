...
/**
 * Application configuration properties for the HRS backend bound from properties with prefix {@code
 * ca.bc.gov.nrs}.
 *
 * <p>This class groups external API addresses and frontend-related settings (including CORS
 * configuration). Instances are populated automatically by Spring Boot's
...
public class HrsConfiguration {
   ...
   *
   * <p>Holds the remote service base URL and an optional API key used to authenticate requests.
   */
  @Data
  @Builder
  @NoArgsConstructor
   ...
   *
   * <p>Contains the configured frontend origin(s) and nested CORS configuration used by the server
   * to allow cross-origin requests from the front-end application.
   */
  @Data
  @Builder
  @NoArgsConstructor
   ...
   *
   * <p>This class contains the allowed headers, allowed HTTP methods and the max age for preflight
   * responses. Values are bound from configuration and are used by the application's CORS setup.
   */
  @Data
  @Builder
  @NoArgsConstructor
  ...
  public static class FrontEndCorsConfiguration {
    ...
    /**
     * List of allowed request headers for CORS (for example, {@code ["Content-Type",
     * "Authorization"]}).
     */
    ...
    /**
     * List of additional allowed origins for CORS, configured separately from the {@code url}
     * property in {@link FrontEndConfiguration}. These origins are used in addition to the
     * configured frontend URL(s) when setting up CORS mappings for API endpoints.
     */
  ...
  public static class CognitoConfiguration {
    ...
    /**
     * Full URL of the Cognito {@code /oauth2/userInfo} endpoint. Defaults to the standard Cognito
