...
public class ForestClientSearchClient {
   ...
   *
   * <p>Returns a pageable result of {@link ForestClientDto}. The total count is read from the
   * {@code X-Total-Count} response header.
   *
  ...
  public Page<ForestClientDto> searchClients(int page, int size, String value) {
    ...
    return new PageImpl<>(
        ...
        Long.parseLong(
            Objects.toString(response.getHeaders().getFirst(BackendConstants.X_TOTAL_COUNT), "0")));
  }
  ...
  private <T> Page<T> paginatedFallback(int page, int size, String value, Throwable ex) {
    log.warn(
        "Fallback for searchClients for {} due to {}.",
        PROVIDER,
  ...
  private List<ForestClientDto> searchClientsByIdsFallback(
      int page, int size, List<String> values, String name, Throwable ex) {
    log.warn(
        "Fallback for searchClientsByIds for {} due to {}.",
        PROVIDER,
        ex == null ? "unknown" : ex.toString());
