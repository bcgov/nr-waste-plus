 ...
 * as well as the standard JPA CRUD operations inherited from {@link JpaRepository}.
 * The custom methods return projection interfaces to avoid loading full entities for
...
public interface ReportingUnitRepository extends JpaRepository<ReportingUnitEntity, Long> {
  ...
  /**
   * Search for user identifiers that match the supplied fragment using an approximate
   ...
   * to compute per-client submission and block counts and the last update timestamp; the
   * method returns a paged projection {@link ClientDistrictSearchProjection}.</p>
   *
