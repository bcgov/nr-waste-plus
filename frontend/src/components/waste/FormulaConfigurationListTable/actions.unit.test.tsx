...
describe('useFormulaConfigurationListRowActions', () => {
  ...
  describe('action structure', () => {
    it('returns a single view-details action for current or past rows', () => {
      const { result } = renderHook(() => useFormulaConfigurationListRowActions(vi.fn()));
  ...
  describe('delete action', () => {
    it('includes a delete action for future-dated open rows', () => {
    ...
    it('omits the delete action for future rows that already have an end date', () => {
      mockIsFutureDated.mockReturnValue(true);
      const { result } = renderHook(() => useFormulaConfigurationListRowActions(vi.fn()));
      const actions = result.current(makeRow({ endDate: '2027-12-31' }));

      expect(actions).toHaveLength(1);
      expect(actions.some((action) => action.id === 'delete')).toBe(false);
    });

    it('omits the delete action for deleted rows', () => {
      mockIsFutureDated.mockReturnValue(true);
      const { result } = renderHook(() => useFormulaConfigurationListRowActions(vi.fn()));
      const actions = result.current(makeRow({ deleted: true }));
    ...
    it('uses a row-specific accessible label for the delete action', () => {
      ...
      expect(resolveTableRowActionValue(actions[1].label, row)).toBe(
        'Delete formula configuration entry',
      );
    });
    ...
    it('calls onDeleteClick with the row when delete is picked', async () => {
      ...
      const { result } = renderHook(() => useFormulaConfigurationListRowActions(onDeleteClick));
  ...
  describe('navigation', () => {
    it('navigates to the detail page on click', async () => {
      ...
      expect(navigateInTree).toHaveBeenCalledWith(mockNavigate, '/configuration/formulas/99');
    });
    ...
    it('navigates with different row ids', async () => {
      ...
      expect(navigateInTree).toHaveBeenCalledWith(mockNavigate, '/configuration/formulas/7');
    });
  });
});
