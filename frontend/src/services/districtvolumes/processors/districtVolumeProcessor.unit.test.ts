...
async function buildXlsxBuffer(rows: unknown[][], sheetName: string, mergeCells?: string[]): Promise<ArrayBuffer> {
...
async function buildCoastFile(): Promise<File> {
  const buf = await buildXlsxBuffer(
    [
      ...
      ['DCK - Chilliwack Natural Resource District', 16.19, 8.87, 5.24, 1.18, 31.48, 17.83, 9.77, 3.87, 1.3, 32.77],
    ],
    'Coast',
    ['A1:A2', 'B1:F1', 'G1:K1'],
  );
